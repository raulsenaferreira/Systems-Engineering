...
def start(**kwargs):
    ...
    for t in range(batches):
        ...
        mostCommonClass = np.argmax(counts)
        predicted = [mostCommonClass] * len(predicted)
        arrPredicted.append(predicted)
        # Evaluating classification
        arrAcc.append(metrics.evaluate(yt, predicted))
        
    
    return "Majority Class Neutral Latency LP", arrAcc, X, y, arrX, arrY, arrUt, arrYt, arrClf, arrPredicted