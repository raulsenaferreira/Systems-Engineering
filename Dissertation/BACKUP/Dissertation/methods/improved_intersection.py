...
def start(**kwargs):
    ...
    print("METHOD: LP as classifier and {} as core support extraction with cutting data method".format(densityFunction))
    ...
    if isBatchMode:
        for t in range(batches):
            ...
            #pdfsByClass = util.pdfByClass2(X, y, Ut, predicted, classes, densityFunction)
            ...
            # ***** Box 6 *****
            #X, y = util.selectedSlicedData(np.vstack([X, Ut]), np.hstack([y, predicted]), selectedIndexes)
    ...
    return "LP + Fixed CSE + KDE past and current instances", arrAcc, X, y, arrX, arrY, arrUt, arrYt, arrClf, arrPredicted