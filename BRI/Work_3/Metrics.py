...
#def main():
def executeEvaluator(path, pathReport, expectedResultsString, results):
    begin = time.time()
    global evaluatorLog
    #logging instantiate
    ...
    writeReport(PK, MAP, DCG, NDCG, F1, path, pathReport)
    
    writeGraphic(path+'/MetricResults/interpolated-precision-recall-11pts.pdf', G11Pts)
    writeMetricsOnFile(path, '/MetricResults/precicionk.csv', PK)
    writeMetricsOnFile(path, '/MetricResults/mean-average-precision.csv', MAP, False)
    writeMetricsOnFile(path, '/MetricResults/discount-cumulative-gain.csv', DCG)
    writeMetricsOnFile(path, '/MetricResults/normalized-discount-cumulative-gain.csv', NDCG)
    writeMetricsOnFile(path, '/MetricResults/f1-measure.csv', F1)
...
def writeMetricsOnFile(path, filename, metric, isDict=True):
...
def writeReport(PK, MAP, DCG, NDCG, F1, path, pathReport):
    evaluatorLog.info('Writing evaluator results on file report...')
    
    f = open(path+pathReport, 'w+')
    f.write("########################    REPORT OF METRICS USING PYLUCENE   ########################\n\n\n")
